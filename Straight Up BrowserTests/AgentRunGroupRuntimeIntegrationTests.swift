...
struct BrowserAgentRunGroupEndToEndTests {
    @Test func parentLaunchesIndependentChildrenAndPersistsSynthesisTree() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("browser-agent-group-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let store = try AgentRunStore(baseDirectory: directory)
        let adapter = DelegationRouterAdapter()
        ...
        agent.submit(
            "Delegate two independent checks",
            pageTitle: "Test",
            pageURL: "",
        ...
        await waitForAgent(
            agent,
            approvingPendingInvocations: true
        )

        #expect(!agent.isRunning)
        #expect(agent.messages.last?.text == "Combined child synthesis")
        let runs = await store.listRuns()
        let root = try #require(runs.first { $0.parentRunID == nil })
        let children = runs.filter { $0.parentRunID == root.id }.sorted {
            $0.id.uuidString < $1.id.uuidString
        }
        #expect(children.count == 2)
        #expect(children.allSatisfy { $0.entryPoint == .childRun })
        #expect(children.allSatisfy { $0.status == .succeeded })
        #expect(children.allSatisfy { $0.runGroupID == root.runGroupID })
        let rootSteps = try await store.steps(runID: root.id)
        #expect(rootSteps.contains { $0.kind == .handoff })
        #expect(rootSteps.contains {
            $0.kind == .stateTransition && $0.summary == "Completed"
        })
        let snapshot = try #require(agent.activeRunGroupSnapshot)
        #expect(snapshot.state == .succeeded)
        #expect(AgentRunGroupTreeProjector.rows(from: snapshot).count == 3)
        #expect(adapter.maximumConcurrentChildren >= 2)
    }

    @Test func attendedRootCreatedPageIsLeasedAcrossNavigationAndRemainsOpen() async throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("browser-agent-page-authority-\(UUID().uuidString)")
        defer { try? FileManager.default.removeItem(at: directory) }
        let store = try AgentRunStore(baseDirectory: directory)
        let page = PageHandle(windowID: UUID(), tabID: UUID())
        let firstURL = "https://first.example/path"
        let secondURL = "https://second.example/next"
        let adapter = PageAuthorityRouterAdapter(
            scenario: .attendedLifecycle(
        ...
        #expect(await executor.toolNames == [
            "new_page", "take_snapshot", "navigate_page", "take_snapshot",
        ])
        #expect(!(await executor.toolNames).contains("close_page"))
        let run = try #require(await store.listRuns().first)
        let steps = try await store.steps(runID: run.id)
        #expect(!steps.contains { $0.kind == .error || $0.kind == .warning })
    }
...
private final class DelegationRouterAdapter: AgentProviderAdapter, @unchecked Sendable {
    let providerID = "delegation-router"
    let capabilities = AgentProviderCapabilities(Set(AgentProviderCapability.allCases))
    private let condition = NSCondition()
    private var activeChildren = 0
    private var peakChildren = 0

    var maximumConcurrentChildren: Int {
        condition.withLock { peakChildren }
    }
    ...
    func events(
        ...
        if request.responseFormat != nil {
            return AsyncThrowingStream { continuation in
                let task = Task {
                    guard self.childStartedAndAwaitOverlap() else {
                        continuation.finish(
                            throwing: DelegationRouterAdapterError.childOverlapTimedOut
                        )
                        return
                    }
                    defer { self.childFinished() }
                    let objective = Self.allText(request).contains("first child")
                        ? "first complete" : "second complete"
                    continuation.yield(.textDelta("{\"summary\":\"\(objective)\"}"))
        ...
        if Self.allText(request).contains("bounded child Runs are terminal") {
            return Self.stream([
                .textDelta("Combined child synthesis"),
                .finished(.stop),
            ])
        }
        if Self.toolResultCount(request) >= 2 {
            return Self.stream([
                .textDelta("Children delegated"),
                .finished(.stop),
            ])
        }
        let first = AgentToolCall(id: "delegate-1", index: 0, name: "delegate_child_run")
        let second = AgentToolCall(id: "delegate-2", index: 1, name: "delegate_child_run")
        return Self.stream([
            .toolCallStarted(first),
            .toolCallCompleted(
                call: first,
                arguments: .valid(Self.delegationValue(objective: "first child"))
            ),
            .toolCallStarted(second),
            .toolCallCompleted(
                call: second,
                arguments: .valid(Self.delegationValue(objective: "second child"))
            ),
            .finished(.toolCalls),
        ])
    }

    /// A deterministic two-party rendezvous proves both child streams are
    /// live concurrently. If production ever serializes child launch, the
    /// first stream times out and the test fails instead of being rescued by
    /// an arbitrary sleep that happens to overlap on a fast machine.
    private func childStartedAndAwaitOverlap() -> Bool {
        condition.lock()
        defer { condition.unlock() }
        activeChildren += 1
        peakChildren = max(peakChildren, activeChildren)
        if activeChildren >= 2 {
            condition.broadcast()
            return true
        }
        let deadline = Date().addingTimeInterval(5)
        while activeChildren < 2, condition.wait(until: deadline) {}
        return activeChildren >= 2
    }

    private func childFinished() {
        condition.withLock { activeChildren -= 1 }
    }
    ...
    private static func delegationValue(objective: String) -> JSONValue {
        .object([
            "objective": .string(objective),
            "allowedTools": .array([]),
            "allowedPages": .array([]),
            "allowedOrigins": .array([]),
            "browserSession": .string("normal"),
            "returnSchema": .object([
                "type": .string("object"),
                "properties": .object([
            ...
            "budget": .object([
                "maximumProviderCostMicrounits": .number(100_000),
                "maximumElapsedMilliseconds": .number(30_000),
                "maximumSteps": .number(4),
                "maximumToolCalls": .number(0),
                "maximumOutputBytes": .number(10_000),
                "maximumChildCreatedPages": .number(0),
            ]),
        ])
    }

    private static func allText(_ request: AgentModelRequest) -> String {
...
private enum DelegationRouterAdapterError: Error {
    case childOverlapTimedOut
}
